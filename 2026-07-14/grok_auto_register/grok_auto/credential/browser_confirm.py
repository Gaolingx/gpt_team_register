...
def create_standalone_page(
    *,
    ...
    """创建铸造用独立浏览器页。始终有头；headless 参数已废弃并忽略。"""
    log = log or _noop_log
    _ = headless  # 兼容旧调用，强制有头
    try:
    ...
    opts = None
    # 仓库根：turnstilePatch 在根目录
    _repo_root = _project_root()
    try:
        # 优先使用本项目 browser.options（已强制有头）
        from grok_auto.browser.options import create_browser_options  # type: ignore

        opts = create_browser_options(profile_key="mint")
        log("using grok_auto.browser.options")
    except Exception as e:  # noqa: BLE001
        log(f"browser options unavailable: {e}")
        opts = None
    ...
    if opts is None:
        ...
        ext = str(_repo_root / "turnstilePatch")
...
def acquire_mint_browser(
    *,
    ...
    """返回 (browser, page, owned)。始终有头；headless 参数已废弃并忽略。

    reuse=True 时浏览器放在线程本地并在任务间清会话。
    """
    log = log or _noop_log
    _ = headless  # 兼容旧调用，强制有头
    st = _mint_tls_get()
    if reuse and st.get("browser") is not None:
        # 代理变化或达到复用次数则回收
        need_recycle = st.get("proxy") != (proxy or None) or (
            recycle_every > 0 and int(st.get("served") or 0) >= recycle_every
        )
        if not need_recycle:
            page = st.get("page")
        ...
        log("mint browser recycle (proxy/served threshold)")
        try:
    ...
    browser, page = create_standalone_page(proxy=proxy, headless=False, log=log)
    if reuse:
        st["browser"] = browser
        st["page"] = page
